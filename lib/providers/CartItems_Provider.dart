...
// Thêm setter cho userId
  ...
  Future<void> initializeWithUserId(int userId) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.1.9:3000/api/cart/getCart/$userId'),
  ...
  Future<void> add(Product product) async {
    ...
    try {
      final url = Uri.parse('http://192.168.1.9:3000/api/cart/addToCart');
  ...
  Future<void> remove(CartItem product) async {
    try {
      final url =
          Uri.parse('http://192.168.1.9:3000/api/cart/deleteCartItem');
  ...
  Future<void> updateQuantity(CartItem product, int quantity) async {
    try {
      final url =
          Uri.parse('http://192.168.1.9:3000/api/cart/updateQuantity');
  ...
  Future<void> clearCart() async {
    try {
      final url = Uri.parse('http://192.168.1.9:3000/api/cart/clear');
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
